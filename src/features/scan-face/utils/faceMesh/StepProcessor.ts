...
export class StepProcessor {
  ...
  processStep1(detection: DetectionResult, canvasWidth: number, canvasHeight: number) {
    ...
    if (validation.canProceed) {
      ...
      } else if ((performance.now() - this.state.step1HoldStart) / 1000 >= CONFIG.TIMING.STEP1_HOLD_SECONDS) {
  ...
  private isValidDetection(detection: DetectionResult): boolean {
    return !!(detection.landmarks && detection.yawDeg !== null && 
              detection.pitchDeg !== null && detection.earValue !== null && 
              detection.marValue !== null);
  }

  private updateBaselines(earValue: number, marValue: number) {
    if (this.state.subPhase === "blink" && (!this.ema.earBase.value || earValue > this.ema.earBase.value * 0.9)) {
      this.ema.earBase.update(earValue);
    }
    if (this.state.subPhase === "mouth" && (!this.ema.marBase.value || marValue < this.ema.marBase.value * 1.1)) {
  ...
  private handleBlink(earValue: number) {
    ...
    if (isBlink) {
      ...
      } else if ((now - this.state.blinkCloseStart) / 1000 >= CONFIG.TIMING.BLINK_MIN_SECONDS) {
  ...
  private handleMouth(marValue: number) {
    ...
    if (isMouthOpen) {
      ...
      } else if ((now - this.state.mouthCloseStart) / 1000 >= CONFIG.TIMING.MOUTH_CLOSE_MIN_SECONDS) {
